...
packages <- c("ape", "devtools", "dplyr", "plyr", "ggplot2", "readr", "ggpubr","ggnewscale",
              "rcompanion", "tidyverse", "tidyr", "ggsignif", "reshape", "ggtree", "phytools", "gridExtra")
...
```{r}
metadata.filtered.markov.subset.long$Host_group <- with(metadata.filtered.markov.subset.long, reorder(Host_group, -jumps))
	ggplot(metadata.filtered.markov.subset.long, aes(x =Host_group, y = jumps, fill=Host_group)) + 
...
```{r}
my_comparisons <- list(c("Cows", "Rabbits"), c("Sheep", "Cows"), c("Sheep", "Rabbits") )
```

## Now subset the data

```{r}
rabbits.cows.sheep.jumps <-subset(metadata.filtered.markov.subset.long, Host_group==c("Cows", "Rabbits","Sheep"))
```

Then, we plot a [violin](https://mode.com/blog/violin-plot-examples/) plot using the function **ggviolin**.

```{r}
ggviolin(data = rabbits.cows.sheep.jumps, x = "Host_group", y = "jumps", fill = "Host_group", color="Host_group", add = "boxplot", add.params = list(fill = "white"))+
